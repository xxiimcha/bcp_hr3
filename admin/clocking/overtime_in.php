...
try {
    ...
    // Check if current time is within the normal shift hours
    if (strtotime($current_time) >= strtotime($shift_start) && strtotime($current_time) <= strtotime($shift_end)) {
        echo json_encode(['status' => 'error', 'message' => 'Overtime In not recorded: Current time is within shift hours.']);
        exit();
    }
    ...
    if ($result->num_rows > 0) {
        // Attendance record exists, update it with the Overtime In details
        $updateQuery = "UPDATE attendance SET overtime_in = ?, status = 'Overtime In' WHERE employee_id = ? AND attendance_date = ?";
        $updateStmt = $conn->prepare($updateQuery);
        $updateStmt->bind_param("sis", $current_time, $employee_id, $attendance_date);
        
        if ($updateStmt->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Overtime In recorded successfully.']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Error recording Overtime In']);
        }
    } else {
        // No attendance record exists; insert a new one
        $insertQuery = "INSERT INTO attendance (employee_id, attendance_date, overtime_in, status) VALUES (?, ?, ?, 'Overtime In')";
